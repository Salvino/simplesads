...
app.get('/encyclopedia/v1/:linguaorigem/:termo', (req, res) => {
	...
	client.search({
		...
		const o = {};
		const key = 'Encyclopedia';
		// const img = 'Imagem';
		o[key] = [];
		for (let i = 0; i < resposta.hits.hits.length; i++) {

			o[key].push({
				Termo: resposta.hits.hits[i]._source.Titulo, Descricao: resposta.hits.hits[i]._source.Descricao, Url: resposta.hits.hits[i]._source.Url, Imagem: resposta.hits.hits[i]._source.Imagem,
			});
...
app.get('/translate/v1/:termo/:linguaorigem/:linguadestino', (req, res) => {
	...
	client.search({
		...
		const tam = (resposta.hits.hits[0]._source.definições.length) - 1;
		// let traducaoJson;
